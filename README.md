<!-- 🌟 PROFILE BANNER -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0A74DA,100:0078D7&height=220&section=header&text=Hi%20There!%20I'm%20Mohammad%20Mounir%20Balqis%20👋&fontSize=30&fontColor=ffffff&fontAlignY=38" />
</p>

<h3 align="center">🚀 Full Stack Web Developer (MERN) | Educator turned Engineer | Lifelong Learner</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/mohammad-balqis/">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-blue?logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:mohammad.balqis284@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-informational?logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/MohammadBalqis">
    <img src="https://img.shields.io/badge/GitHub-Profile-black?logo=github&logoColor=white" />
  </a>
  <img src="https://komarev.com/ghpvc/?username=MohammadBalqis&label=Profile%20Views&color=0A74DA&style=flat-square" />
</p>

---

## 👨‍💻 About Me  

Hi, I’m **Mohammad Mounir Balqis** — a **Full Stack Developer** with a foundation in education and a strong passion for digital innovation.  

After several years in teaching, I transitioned into web development and successfully completed multiple production-grade projects using the **MERN stack** and **modern web technologies** through **The Digital Hub – Beirut Souks Program**.  

Today, I specialize in building **data-driven dashboards, POS systems, and educational web platforms** that merge functionality with design.  

💡 *I believe technology can empower learning and simplify business management.*

---

### 💼 Professional Highlights  

- ✅ **Completed:** Vehicle Dealership Management & POS System (MERN)  
  A comprehensive enterprise-grade system handling **inventory, accounting, and transactions** with multi-role dashboards for admin, accountant, and clerk.

- ✅ **Completed:** The Digital Hub Website  
  An official showcase website for The Digital Hub program, highlighting projects, tech tracks, and youth innovation in Lebanon.

- 🎯 Transitioned from **English Educator → Full Stack Developer** in under 6 months through structured self-learning and project-based practice.  
- 💬 Focused on creating digital tools that solve **real business and educational problems.**

---

## 🧠 Tech Stack  

<p align="center">
  <img src="https://skillicons.dev/icons?i=react,nodejs,express,mongodb,html,css,javascript,bootstrap,git,github,figma,vscode" />
</p>

---

## 🚀 Completed Projects  

### ⚙️ [POS System for Vehicles](https://github.com/MohammadBalqis/POS-system-for-Vehicles)
A full **Vehicle Dealership Management and POS System** built with the **MERN stack**.
- 🔐 Role-based dashboards: **Admin, Accountant, Clerk**  
- 💵 Modules: Sales, Expenses, Income, Payroll, Transactions  
- 📊 Features: Charts, Analytics, and Automated Calculations  
- 🧰 Tech Used: MongoDB, Express.js, React, Node.js  
- 🎨 Focused on clear UX/UI, reliability, and data integrity  

---

### 💻 [The Digital Hub Website](https://github.com/MohammadBalqis/The-Digital-Hub-Website)
A professional website representing the **Digital Hub Program** in Beirut Souks.  
- 🌍 Responsive and interactive frontend with **React**  
- 📄 Showcases the Hub’s training programs, web projects, and achievements  
- 🧩 Optimized for speed, accessibility, and maintainability  
- 🧠 Demonstrates strong project structure and modular component design  

---

## 📈 GitHub Performance  

<p align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=MohammadBalqis&show_icons=true&theme=tokyonight&hide_border=true" />
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=MohammadBalqis&layout=compact&theme=tokyonight&hide_border=true" />
</p>

<p align="center">
  <img height="165" src="https://streak-stats.demolab.com?user=MohammadBalqis&theme=tokyonight&hide_border=true" />
</p>

---

## 🏆 Achievements & Trophies  

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=MohammadBalqis&theme=tokyonight&no-frame=true&margin-w=8" />
</p>

---

## 🌍 Connect with Me  

<p align="center">
  <a href="https://www.linkedin.com/in/mohammad-balqis/"><img src="https://img.shields.io/badge/LinkedIn-blue?logo=linkedin&style=for-the-badge" /></a>
  <a href="mailto:mohammad.balqis284@gmail.com"><img src="https://img.shields.io/badge/Gmail-red?logo=gmail&style=for-the-badge" /></a>
  <a href="https://github.com/MohammadBalqis"><img src="https://img.shields.io/badge/GitHub-black?logo=github&style=for-the-badge" /></a>
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0A74DA,100:0078D7&height=100&section=footer" />
</p>
